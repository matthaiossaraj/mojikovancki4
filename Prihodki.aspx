...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h1 class="inMainTitle">Prihodki</h1>
        <div>
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:moneyConnectionString %>" ProviderName="<%$ ConnectionStrings:moneyConnectionString.ProviderName %>" SelectCommand="SELECT income.amount AS Znesek, income.`date` AS Datum, accounts.name AS Racun, categories.name AS Kategorija FROM income INNER JOIN accounts ON accounts.id = income.account INNER JOIN categories ON categories.id = income.category"></asp:SqlDataSource>
        </div>
